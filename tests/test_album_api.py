...
def _auth(telegram_id: int) -> dict:
    # Токен — внутри теста, а не при импорте: в CI нет .env, и секрет подписи
    # подставляет conftest уже после сбора модулей (красный CI 16.09).
...
def test_album_routes_are_behind_paywall(client):
    assert client.get("/search/live/albums?q=2004", headers=_auth(902)).status_code == 402
    assert client.get("/albums/live/5", headers=_auth(902)).status_code == 402
    assert client.post("/albums/live/5/library", headers=_auth(902)).status_code == 402
...
def test_search_albums_returns_cards(client, monkeypatch):
    ...
    data = client.get("/search/live/albums?q=скриптонит 2004", headers=_auth(901)).json()
...
def test_album_tracks_have_stream_refs(client, monkeypatch):
    ...
    items = client.get("/albums/live/970298674", headers=_auth(901)).json()["items"]
...
def test_album_not_found_and_bad_ids(client, monkeypatch):
    ...
    assert client.get("/albums/live/5", headers=_auth(901)).status_code == 404
    assert client.get("/albums/live/0", headers=_auth(901)).status_code == 404
    assert client.get(f"/albums/live/{10**15}", headers=_auth(901)).status_code == 404
...
def test_add_album_queues_quietly_once(client, monkeypatch):
    ...
    first = client.post("/albums/live/77/library", headers=_auth(901))
    ...
    assert client.post("/albums/live/77/library", headers=_auth(901)).status_code == 409
    assert len(queued) == 1
