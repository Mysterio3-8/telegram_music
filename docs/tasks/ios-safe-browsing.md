...
**Статус:** 🔴 не решено. Диагностика проведена 08.08 — нужен переезд на новый домен
...
**Шаг 1 — СДЕЛАН (08.08).** Владелец проверил на iPhone: **ругается везде** —
и в Safari напрямую, и через бота.

Вывод однозначный: это Safe Browsing / WebKit, а не проверка Telegram. Значит
@BotSupport не поможет, а **переезд на чистый домен поможет**. Правки от 04.08
(404 вместо 200 на любой путь, содержимое в `#app`) предупреждение не сняли —
но они нужны, иначе новый домен унаследует ту же репутацию за пару недель:
сканеры находят свежие домены быстро.
