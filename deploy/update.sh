...
# ⚠️ -d '\n' обязателен. Без него xargs режет ключи по ПРОБЕЛАМ, а ключ кэша —
# это нормализованный запрос человека («livesearch:кизару фейк айди»). То есть
# сброс работал только для однословных запросов, а для всех остальных молча не
# делал ничего: 14.08 из 20 ключей удалилось 2. Ровно это и создавало иллюзию
# «деплой не дошёл» — правки поиска были на месте, а человек три часа видел
# старую выдачу из кэша.
deleted=$(redis-cli --scan --pattern 'livesearch:*' | xargs -d '\n' -r -n 100 redis-cli del | paste -sd+ | bc 2>/dev/null || echo 0)
