...
**➡️ Главный список работ — [CHECKLIST.md](CHECKLIST.md)**: что болит, что в работе, что отложено. Каждая крупная задача ссылается на своё ТЗ в [docs/tasks/](docs/tasks/). Новую сессию начинай с него.

**Хронология и разборы инцидентов — [NEXT_SESSION.md](NEXT_SESSION.md)** — приоритеты (бот не должен падать, деньги, поисковый парсер) и полный список забытого/отложенного.
