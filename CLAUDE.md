...
1. **Архивный чат** — минт идёт `send_audio` в `TELEGRAM_ARCHIVE_CHAT_ID`, а
   новый бот не может писать тому, кто не нажал у него /start. Пока не нажал —
   `Bad Request: chat not found`, и **минт не работает совсем**: ни импорт новых
   треков, ни восстановление старых. Настоящее лечение — завести под архив
   отдельный приватный канал и вписать его id вместо человека.
