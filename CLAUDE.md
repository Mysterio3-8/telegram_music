...
- ⚠️ **Живая проверка локально:** «kizaru fake id» — 13 результатов за 2.6 сек,
  «lxst cxntury падение» и «кизару» находятся. Но на этой машине **нет curl_cffi-
  маскировки**, поэтому SoundCloud отвалился и вся выдача пришла с YouTube. Поиск
  честно деградировал на один источник — так и задумано. **На VPS обязательно
  проверить, что SC отвечает** (`journalctl -u tg-music-api | grep источник`):
  молчаливый отказ SC оставляет выдачу на одном YouTube, поэтому отказ теперь
  логируется громко
