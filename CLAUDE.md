...
- **Осталось из SPEC-КАТАЛОГ**: срез 5 (таблица `albums` + импорт SoundCloud sets как альбомов + автоплейлисты по жанрам) — отложен, альбомы в карточке уже работают из `tracks.album`; периодический `bind-tracks` (можно в ежедневный таймер); папка `проверка/` со скринами — сверка дизайна карточки, когда владелец скинет
