...
- ✅ **keybest.cc — флаг Google Safe Browsing снят** (2026-07-21 подтверждено: Search Console «No issues detected» + Transparency Report «Не найдено небезопасного контента», обновлено 26.06.2026). Владение доменом подтверждено через Dynadot (NS `dyna-ns.net`) TXT-записью. Если предупреждение «Deceptive Website Warning» всплывёт на чьём-то устройстве при чистой базе — это локальный кэш Safe Browsing на самом устройстве (Safari/Chrome хранят копию базы, обновляется не мгновенно): Настройки → Safari → выключить/включить «Fraudulent Website Warning» форсирует пересинхронизацию. Проверить живой статус: `https://transparencyreport.google.com/safe-browsing/search?url=keybest.cc`. Апелляция в обход Search Console: `https://safebrowsing.google.com/safebrowsing/report_error/?url=https://keybest.cc`. Зона `.cc` в группе риска — если флаг вернётся, переезжать на `.ru`/`.com`
...
Решение владельца: каталог на 1 млн треков не копим, поиск идёт **сразу в
источники**. Спека — [docs/superpowers/specs/2026-08-02-live-search-design.md](docs/superpowers/specs/2026-08-02-live-search-design.md).

**Корень старой жалобы «поиск слабый, андеграунд не находится»:** бот шёл в сеть,
только если локальная база вернула НОЛЬ строк. Поиск по подстроке почти всегда
что-то возвращает, поэтому живой поиск не срабатывал практически никогда — человек
видел пять похожих мусорных совпадений вместо своего трека.

- **Ядро** — `search_candidates()` в [track_lookup/__init__.py](app/services/track_lookup/__init__.py):
  SoundCloud и YouTube опрашиваются **параллельно** (`asyncio.gather`), время ответа
  равно медленному из двух. [merge.py](app/services/track_lookup/merge.py) —
  SC целиком и первым, YouTube добавляет только то, чего в SC нет; дедуп по
  нормализованной паре «артист + название» через транслит, поэтому «Kizaru — Fake ID»
  с YouTube опознаётся как тот же трек, что «кизару фейк айди» с SoundCloud
- **Кэш выдачи** [search_cache.py](app/services/search_cache.py): Redis, TTL 3 ч,
  ключ — нормализованный запрос. Redis лёг → поиск работает, просто медленнее
- **Бот** [quick_search.py](app/handlers/quick_search.py): список кандидатов кнопками,
  кандидаты в FSM (в `callback_data` 64 байта — ссылка SoundCloud туда не влезает),
  кнопка ссылается на индекс. Трек уже в базе → мгновенно по file_id; нет → задача
  `search.fetch_candidate` в очередь `youtube_user`. **Скачивание в воркере, не в боте** —
  ffmpeg на 961 МБ уже дважды ронял прод по OOM
- **Mini App**: `GET /search/live` отдаёт кандидатов с подписанным ref
  ([candidate_ref.py](app/services/candidate_ref.py) — HMAC, самодостаточный, ничего
  не хранится), `GET /stream/{ref}` проксирует поток источника с Range → играет
  через ~2 сек, фоном ставится заливка в Telegram, дальше трек мгновенный у всех
- **Миксы без каталога** [shelves.py](app/services/shelves.py): полка = набор
  сид-запросов к SoundCloud, результаты чередуются. Личный микс — те же полки плюс
  артисты из `track_events` пользователя. `GET /shelves`, `/shelves/{slug}`,
  `/shelves/mix/personal`
- **Охват вместо чистоты** (выбор владельца): `search_min_seconds` **60 → 0** —
  фристайлы, скиты и интерлюдии короче минуты это и есть андеграунд. Верхний порог
  900 сек остался. Если ранжирование отбросило всё, отдаём то, что вернул источник,
  а не пустой экран
- **Чистка каталога** `python -m app.cli.cleanup_catalog` — по умолчанию только
  показывает числа, удаляет по `--apply`: клипы, длиннее 15 минут, без обложки.
  `--fingerprints` стирает отпечатки (их индекс — 128 МБ, ~40% базы). Короткие треки
  СПЕЦИАЛЬНО не трогаются: удалять то, что поиск теперь считает валидным, — противоречие
### 🔴 ГЛАВНАЯ НАХОДКА — SoundCloud был выключен мёртвыми прокси

Разведка прода показала: **все семь прокси из `PROXY_LIST` отвечают Connection
refused**. SoundCloud ходил только через них, поэтому приоритетный источник поиска
молча отдавал ноль результатов — **вот настоящая причина «поиск слабый, мало
треков, андеграунд не находится»**, а не алгоритм. Напрямую SC отвечает нормально.

Чинилось в три захода, каждый вскрывал следующий слой:
1. `attempt_plan()` — последняя попытка **всегда без прокси**. Мёртвый пул больше
   не может выключить источник.
2. Не помогло: `ignoreerrors=True` заставляет yt-dlp гасить сетевую ошибку самому и
   возвращать словарь с **пустым `entries`**, а не `None`. Снаружи это неотличимо
   от «ничего не найдено», поэтому код возвращался с первой попытки, не доходя до
   прямой. Теперь пустой ответ — тоже повод пробовать дальше.
3. Скорость: попыток через прокси стало **одна вместо трёх** (19 сек → 6), плюс
   `sleep_interval_requests` выключен для живого поиска — это анти-бан фонового
   обхода профилей, а тут ответа ждёт человек. Итог **1.9–4.7 сек**, SC=15 в выдаче.

⚠️ **Прокси в `.env` мёртвые — их либо менять, либо чистить `PROXY_LIST`.** Пока они
там, каждый запрос тратит одну впустую попытку. Код от них больше не зависит.

### Три бага, найденных владельцем на живом тесте

- **«✅ Исполнитель — Секс» вместо «Lida — Секс».** `_download_soundcloud_once`
  клал uploader только во второй элемент кортежа, а `download_candidate` кортеж
  отбрасывал — внутри `DownloadedAudio` автор был пуст. У SoundCloud название часто
  без автора («Секс»), автор берётся именно из uploader. Плюс `import_candidate`
  теперь добирает автора и обложку из выдачи поиска, если скачивание их не отдало
- **Трек приходил дважды.** Минт `file_id` — это реальная отправка файла в архивный
  чат, а `TELEGRAM_ARCHIVE_CHAT_ID` не задан → архивом становится чат первого
  админа. Владелец получал копию минта и нашу. Сейчас в этом случае шлём только
  текст. ⚠️ **Настоящее лечение — завести отдельный канал под архив** и вписать его
  id; пока не задан, у админов минт всегда светится в личке
- **Медленно** — см. пункт 3 выше
- **Трек без обложки в плеере Telegram.** Обложка БЫЛА вшита в файл (проверено:
  APIC на 1.1 МБ), но Bot API берёт превью **отдельным полем `thumbnail`** и требует
  JPEG до 200 КБ со стороной до 320 px — крупную вшитую картинку плеер молча
  игнорирует. Теперь миниатюра отдаётся явно: тот же арт SoundCloud в 200×200
  (`thumbnail_soundcloud_artwork`). Проверено вживую: `msg.audio.thumbnail` = 200×200.
  Попутно регекс размеров не ловил суффикс `-original`, поэтому в Mini App грузился
  исходник больше мегабайта вместо `t500x500`.
  ⚠️ Миниатюра появляется только у **заново заминченных** треков: у старых
  `file_id` уже без превью, перезалить их можно только повторным минтом

**Прокси очищены** (`PROXY_LIST=` пусто, бэкап `.env.bak-*` на сервере). Теперь
`attempt_plan() == [False]` — одна прямая попытка, без потерянной впустую.

**Архив треков — второй админ** (`TELEGRAM_ARCHIVE_CHAT_ID=7446911479`, решение
владельца): минт больше не светится у владельца в личке. Чат проверен, бот его
знает. ⚠️ Если этот пользователь заблокирует бота, минт перестанет работать
целиком — загрузка треков встанет у всех.

### YouTube стал фолбэком, а не источником каталога

Запрос «секс» приносил ролики про игрушки, биологию и трейлер сериала: YouTube —
видеохостинг, и на общий запрос честно отдаёт видео по теме.

- YouTube подключается, **только если SoundCloud набрал меньше**
  `youtube_fallback_min_results` (5). SC почти всегда набирает 15, так что YouTube
  нужен редко — заодно быстрее и меньше риск его антибота
- `looks_like_music()` фильтрует **форму, а не тему** (в песнях бывает что угодно):
  берём, если канал «Исполнитель - Topic» (автоматический музыкальный канал) или
  название в виде «Исполнитель - Название». Тире нормализуется — их у людей
  штук восемь разных, и «Fake ID – kizaru» иначе не опознавался
- `Candidate.uploader` **отдельно от `artist`**: имя канала нельзя пускать в
  сопоставление, иначе ломается дедуп одного трека между YouTube и SoundCloud
- Маркеры не-музыки расширены: интервью, подкаст, лекция, влог, гайд, распаковка,
  «что такое», «топ 10», плюс кино — teaser, sneak peek, season, эпизод, S01E01
- Замер после фикса: «секс» с YouTube — 8 роликов, прошло 2, оба музыка

### Чистка каталога выполнена

`cleanup_catalog --apply --fingerprints`: **30 157 → 5 951 трек**, база
**323 МБ → 50 МБ** (после VACUUM), диск +400 МБ. Отпечатков стёрто 5 946.

⚠️ **Защита данных людей включена по умолчанию.** Под критерии подходило 26 641,
но 2423 из них лежали в чьих-то плейлистах — 93% всего собранного пользователями.
Живой поиск такой трек найдёт заново, а **дыру в плейлисте не восстановит**:
восстанавливать нечего, запись удалена вместе с названием. Поэтому удалено 24 206,
плейлисты (2610 треков, 140 штук) и библиотеки не тронуты. Снять защиту —
`--include-user-tracks`, но это осознанная потеря.

## Checkpoint (2026-08-02) — «бот опять тормозит» оказался экраном /admin

Мидлварь замера окупилась в первый же день: за 3 часа она поймала **ровно три**
медленных обработки, все от админа и все в админке — `/admin` 5.1 сек,
`adm:stats` 4.7 и 4.1 сек. Действий обычных пользователей в логе нет вовсе,
то есть бот в целом не медленный, а владелец судил по единственному экрану,
которым пользуется чаще всего.

**Причина.** Запросы с условием `storage_path IS NOT NULL` шли полным сканом
`tracks` (133 МБ): `count_reclaimable` делал это дважды (count и sum отдельными
запросами), плюс `archived_on_disk` ещё раз. Подходящих строк при этом 52,
а у `count_reclaimable` — ноль. БД 323 МБ при 961 МБ памяти, страничный кэш
вытесняется, поэтому каждый заход в админку перечитывал таблицу с диска.

**Лечение:** частичный индекс `ix_tracks_archived` (миграция `e3cb5860dad1`) —
NULL-ы не индексируются, поэтому он весит **4 КБ**; план сменился со `SCAN tracks`
на `SEARCH ... USING COVERING INDEX`. Плюс count и sum склеены в один запрос.

**Замер (сброшенный кэш страниц): 1.51 сек → 0.22 сек**, на прогретом 0.08 сек.

Остальные счётчики экрана проверены планами и идут по покрывающим индексам
(клипы — `ix_tracks_title` на 1 МБ, всего треков — `ix_tracks_artist_id`) —
их трогать не потребовалось.

⚠️ Попутное наблюдение: `ix_tracks_fingerprint` весит **128 МБ** — почти столько
же, сколько сама таблица, и это ~40% всей базы. Отпечатки длинные, индекс нужен
для дедупа при загрузке. Если понадобится ужать базу — смотреть сюда первым
делом (например, хранить укороченный префикс отпечатка).
...
**Симптом:** бот не отвечал на /start, лагал. **Причина:** диск VPS заполнился до 100%
(0 байт) → Redis не смог сделать RDB-снимок → `stop-writes-on-bgsave-error` заблокировал
запись → FSM-хранилище бота (Redis) падало на каждой команде.
**Что заполнило диск:** одновременный запуск тяжёлых фоновых задач на крошечном боксе
(961 МБ RAM, 20 ГБ диск, общий с news-rewriter): `recover_covers --limit 30000` (yt-dlp
плодил temp в /tmp — 927 МБ) + `discover_loop` + celery + 2 бота.
**Триаж:** убил recover_covers/discover/research/recover-задачи; почистил /tmp (927М),
journal (700М), старые бэкапы; `redis-cli config set stop-writes-on-bgsave-error no` +
`bgsave` + `config rewrite` (постоянно — для FSM снимок некритичен); рестарт бота. Живой.
**ВЫВОД:** этот бокс НЕ тянет агрессивный парсинг. **recover_covers 30k и discover_loop
НЕ перезапускать** на нём — снова уронят. Диск 94% (1.3 ГБ). Нужен **больший сервер**
(это и есть блокер для 1М/PostgreSQL — теперь с доказательством: переполнение уронило прод).

**РЕЦИДИВ (2026-07-27):** бот падал ещё дважды — тот же discover_loop продолжал
плодить `research country ...` и с recover-задачами выжирал RAM (961МБ) → OOM убивал
Redis → бот. Окончательно: убил discover_loop и все осиротевшие research/recover
(cron/systemd-юнита для них НЕТ — больше не респавнятся), почистил /tmp, **снизил
soundcloud-воркер concurrency 3→1** (коммит `0a5b411`), защита диска задеплоена
(`min_free_disk_mb`, коммит `0fda45a`). После этого нагрузка 9→1.75, стабильно.
**НЕ запускать discover_loop/recover_covers/высокий concurrency на этом боксе** —
только после переезда на больший сервер. Массовый парсер сейчас: 1 поток, с диск-гейтом.

## Checkpoint (2026-07-26, поздний вечер) — доп. задачи владельца (321 тест)

Пачка конкретных багов/фич от владельца — сделано и задеплоено (коммит `6ee8894`):
- **Плеер** ([state.js](miniapp/src/state.js)): полный MediaSession (play/pause/seek/next/prev + обложка + playbackState), восстановление воспроизведения при возврате из фона (`visibilitychange`), фиксация `audio.volume=1` от «затихания». Автоплей подряд (`ended→playNext`) уже был. ⚠️ у тех, кто включал эквалайзер, фон/громкость всё равно страдают — Web Audio на iOS душит в фоне, `createMediaElementSource` необратим до перезагрузки; для большинства (без EQ) — ок
- **Быстрый поиск в боте** ([quick_search.py](app/handlers/quick_search.py)): любой текст боту → трек аудиосообщением (локальный поиск → скрытый веб-парсер). Зарегистрирован после FSM в [main.py](app/main.py), `~F.text.startswith("/")`
- **SC-only подтверждён** (владелец повторил): массовый парсер только SoundCloud; YouTube только в поисковом парсере для треков. Удалено **318 клипов/видео** (`delete_clip_tracks`), 21 YT-источник погашен ранее
- **Восстановление обложек**: `soundcloud.fetch_cover_url` + [recover_covers.py](app/cli/recover_covers.py). На VPS было **25864 трека без обложки** — запущен фоновый прогон (`/var/log/tg-music-covers.log`, setsid)
- **Прогресс к цели** 1М треков / 10к артистов — в админ-статистике (`ProjectStats.artists_total`)
- Дальше: **завтра продолжаем по SPEC-2.0** (решение владельца)

## Checkpoint (2026-07-26, вечер) — ВЕСЬ ПЛАН A–G ЗАКРЫТ (317 тестов)

**A, B, C(+усиление), D, E, F, G-бэкап — всё на проде.** Осталось из G только крупная инфра (PostgreSQL, 1М) — нужна координация с владельцем (даунтайм, ресурсы).

- **Блок F — поддержка (коммит `a2e52cf`):** бот @suptgmusic_bot ([support_bot.py](app/support_bot.py), свой polling, юнит `tg-music-support.service`, живой). Категории жалоба/отзыв/идея, приём текста+медиа (copy_message владельцу), антиспам (min 15 симв + 3/час). Кнопка в меню бота и Mini App. ⚠️ **владелец должен нажать /start у @suptgmusic_bot** — иначе Telegram не даст боту слать ему обращения. См. [[support-bot-secret]]
- **Усиление 24/7-парсера SC-only (коммит `a2e52cf`):** `youtube disable-all` погасил 21 YouTube-источник (с ютуба лез мусор/клипы); scan-due и так SC-only. Чистка клипов: `count/delete_clip_tracks` по маркерам названия, кнопка «🎬 Клипы/видео» в админке
- **Блок G-бэкап (коммит `cc7886f`):** [backup.py](app/services/backup.py) консистентный онлайн-снимок SQLite + ротация (`BACKUP_KEEP`), ежедневный таймер `tg-music-backup` 04:00. Первый снимок сделан (270 МБ)
- ⚠️ **ДИСК VPS 92% (1.7 ГБ свободно)** — VPS общий с news-rewriter, БД 270МБ, audio_cache 1ГБ. `BACKUP_KEEP=2` на VPS чтобы не забить. Почистил journald (−736МБ). Это подтверждает необходимость **PostgreSQL + больший диск/offsite-бэкап** (остаток G) — on-disk бэкап не переживёт потерю сервера. Нужна координация с владельцем
- ⚠️ **Живой рекуррент (E1) владелец должен проверить реальной картой** — деньги, тестом не покрыто
...
- **Усиление C ЗАДЕПЛОЕНО (коммиты `2c9026e`, `6098691`):** поиск переведён на ранжирование `track_lookup` (`find_track`/`import_by_query`) вместо первого результата; порог понижен (0.3) + фолбэк на топ-кандидата → выдаёт трек даже по 1 слову/букве/артисту (проверено вживую: «кизару»→«Кизару - Зеркало» с SoundCloud). `download_audio(as_mp3=)` возвращён. **Автодискавери запущен на VPS** ([deploy/discover_loop.sh](deploy/discover_loop.sh), setsid, лог `/var/log/tg-music-discover.log`): страны+жанровые сиды→attach-sources (SC-only), бесконечный цикл к 10k. Было 5142 артиста (1699 SC); legacy-YouTube источники вычищены (`research disable-youtube-sources`)
- **ОСТАЛОСЬ: F (поддержка), G (надёжность).** F — бот @suptgmusic_bot ([[support-bot-secret]], токен в .env). G — бэкап БД, PostgreSQL, 1М, ускорение (крупная инфра, нужна координация с владельцем: даунтайм миграции, ресурсы). ⚠️ Владелец должен проверить живой рекуррент реальной картой перед доверием автосписанию
...
- **Блок A — ЗАДЕПЛОЕН** (коммит `ee4e04d` + авто-коммит `8760efa` с main.js/components.css/models.py):
  - **Ползунок плеера** — pointer-drag с зоной касания 34px (было: тап по 4px, `case "seek"` в main.js). `isSeeking` глушит обновление из аудио во время перетаскивания
  - **Обложки 1:1** — в плеере `object-fit:contain` + размытая заливка по бокам (`renderCover(...,withBlur=true)`, класс `cover-art--fit`, слой `.cover-blur`); в списках оставлен `cover` (46px, дёшево). Апскейл артворка SoundCloud → t500x500 (`upscale_soundcloud_artwork` в soundcloud.py)
...
## Checkpoint (2026-07-30) — УСТОЙЧИВОСТЬ, АНТИСПАМ, РАЗБОР «УСКОРЕННОГО ЗВУКА» (352 теста)

- **Бот не падает**: глобальный обработчик ошибок [handlers/errors.py](app/handlers/errors.py) — исключение в любом хендлере пишется с traceback (`journalctl -u tg-music-bot | grep ERROR`), пользователь получает внятный ответ вместо тишины. `Restart=always` + `OOMScoreAdjust=-800` в юните уже были
- **Антиспам/DDoS**: [middlewares/throttling.py](app/middlewares/throttling.py) — интервал 0.7 сек, не больше 12 действий за 10 сек, при флуде пауза 20 сек, предупреждение **один раз на серию** (иначе сами флудим), админы без лимитов. Зарегистрирована **первой** — флуд гасится до БД, подписки и поиска. Счётчики в памяти (в Redis намеренно не пишем: запись на диск при флуде уронила прод 2026-07-26)
- **Лаги и «database is locked»**: SQLite переведён на **WAL** + `busy_timeout=30000` + `synchronous=NORMAL` ([db/base.py](app/db/base.py)). Один файл БД делят бот, API и 6 воркеров — при journal_mode=DELETE писатель блокировал читателей. ⚠️ 7 файлов задач создавали свой engine **без PRAGMA** — введён `build_task_engine()`, все переведены на него. Проверено на проде: `journal_mode=wal`, появился `music_bot.db-wal`
- **«Аудио играют ускоренно» — на сервере причины нет** (проверено, не догадка):
  1. в коде нет ни одного фильтра темпа (`atempo`/`asetrate`) — grep по всему `app/` пуст;
  2. файлы корректны: 44100 Гц, стерео, **все 40 проверенных** с одинаковой частотой (гипотеза 48000↔44100, дающая ровно +8,8%, опровергнута);
  3. длительность в БД совпадает с фактической (179↔179.8, 190↔190.8, 60↔60.1) — при ускорении файла она была бы короче;
  4. Mini App не трогает `playbackRate` (grep пуст).
  **Вывод: ускоряет клиент.** В плеере Telegram есть регулятор скорости (1x/1.5x/2x) — он запоминается глобально и применяется ко всем аудио; достаточно один раз случайно нажать. Проверить: открыть плеер трека в Telegram → выставить **1x**
- **Форматы**: в базе 29 076 m4a (массовая закачка) и 1 073 mp3 (поисковый парсер — новые уже mp3, как просил владелец). Перевод всей базы в mp3 — отдельная тяжёлая операция (файлы живут у Telegram по `file_id`, локально только LRU-кэш → надо скачать/переконвертировать/переотправить 29k треков), делать после переезда на больший сервер

## Checkpoint (2026-07-29) — ПОИСК ЗА СЕКУНДЫ + НОВАЯ МОНЕТИЗАЦИЯ (343 теста)

Приоритет владельца: «любой трек в мире за секунды в mp3» + «бот бесплатный, Mini App за 29 ₽». Всё задеплоено.

- **Поиск по любой букве/автору, кириллица** ([search_index.py](app/services/search_index.py)): `tracks.search_index` — «исполнитель название» в нижнем регистре + транслит, миграция `0e5ddc09e7e6`, бэкфилл 30 148 треков. **Корневой баг**: SQLite `lower()`/`ILIKE` понижают только ASCII, поэтому запрос «ки» русскими буквами находил 0. Теперь: «к» → 18 740, «кизару» → 240 (и находит латинские `Kizaru`), «би» → 2151. Бэкфилл: `python -m app.cli.reindex_search`
- **Парсеры разведены** (решение владельца): массовый 24/7 — **только SoundCloud** (`attach_source_for_artist` больше не заводит YouTube → `no_source`; чистка прошлых: `research disable-youtube-sources`). Поисковый — SoundCloud, YouTube только фолбэком
- **Всегда mp3 + «только треки»**: `download_soundcloud_audio(as_mp3=True)` (ffmpeg), обложка оригинальная. Фильтр длительности `search_min_seconds=60`/`search_max_seconds=900` — подобрано по реальной базе через **[app/cli/duration_stats.py](app/cli/duration_stats.py)** (в базе: <60 сек — 10% обрезков, >15 мин — 4,3% подкастов). Длительность SC берётся из выдачи → мусор отсекается **до** скачивания
- **Качество выдачи**: штраф пере-версиям (slowed/reverb/cover/nightcore), если их не просили явно — оригинал побеждает; автор аплоада учитывается в ранжировании (официальный трек вместо чужого реаплоада)
- **Выдача в боте списком** ([quick_search.py](app/handlers/quick_search.py) + [keyboards/quick_search.py](app/keyboards/quick_search.py)): кнопки «2:19 Артист - Название», пагинация «1/6», тап → аудио. Формат по скринам владельца
- **Деньги**: **29 ₽/мес** вместо 49 ₽, тарифы 1/3/6/12 (29/87/174/348), **Stars скрыты** из интерфейса (код приёма оставлен), триал **3 → 1 день**. Пэйвол Mini App ([paywall.js](miniapp/src/screens/paywall.js)): бот бесплатный, приложение по подписке, «1 день бесплатно». ⚠️ **Админам `is_premium_active` всегда True** — иначе владелец упирается в собственный пэйвол
- **Тексты**: `/start` переписан (поиск в чате бесплатно, плеер 29 ₽), кнопка меню «Открыть плеер — 29 ₽/мес», кнопка переноса убрана из Mini App (осталась в боте). Пост для канала — верхний в [NEWS.md](NEWS.md), **со списком нужных скринов** (владелец снимает сам)
- ⚠️ Осталось: скрины к посту от владельца; проверить живой платёж 29 ₽ картой

## Checkpoint (2026-07-25) — ДОРАБОТКА ПО РЕФЕРЕНСАМ (папка проверка/): артисты, поиск, шит трека

90 скринов Яндекс/VK Музыки из `проверка/` разобраны (5 параллельных агентов). Владелец: «Обзор» и «Книги и шоу» НЕ делаем (3 таба). Реализовано и **задеплоено**:
- **A. Похожие артисты + «Слушать похожее»** ([artist_follow.py](app/services/artist_follow.py) `similar_artists` — по общим жанрам, добор земляками; кнопка микса по артисту в карточке)
- **B. Подписка на артиста** — таблица `user_artists` (миграция `19f13d7bd996`), `POST/DELETE /artists/{id}/follow`, `GET /my/artists`, кнопка-toggle в карточке + экран «Мои артисты» ([myartists.js](miniapp/src/screens/myartists.js)) в библиотеке (отдельно от локальных favorite_artists для рекомендаций)
- **C. Шит трека — полный набор** ([trackSheet.js](miniapp/src/components/trackSheet.js)): в плейлист (шит-пикер [playlistPicker.js](miniapp/src/components/playlistPicker.js) + создание), в очередь (`addToQueue`), следующим (`playNextInQueue`), к артисту. `POST /playlists/{id}/tracks/{track_id}`
- **D. Поиск секциями** ([search.py](app/services/search.py) `search_all`, `GET /search/all`): Артисты / Альбомы / Плейлисты / Треки отдельными блоками. Альбомы матчат по названию ИЛИ артисту (в выдаче по имени видны его альбомы)
- **E. Карточка артиста** ([artist_card.py](app/services/artist_card.py)): «Последний релиз» блоком, «Синглы» (треки без альбома), развёрнутый «Топ треков» (кнопка «Все N»), похожие лентой
- Тесты: **263 passed** (+11). Проверено в dev-браузере DOM-ами: карточка (жанры/подписка/релиз/похожие/синглы/альбом), подписка→«Мои артисты», секционный поиск, все пункты шита
- **Осталось из референсов** (по желанию владельца): радиостанции, лента друзей (нужен соцграф), гранулярные уведомления, приватный аккаунт, дизлайк трека — не блокеры, отдельными срезами
...
- **ЗАДЕПЛОЕНО НА ПРОД (2026-07-23 вечер)**: миграции `66dd24c3fa53` + `227271058451` применены, 324 жанра посеяны, все сервисы active, Mini App с жанрами/карточками живой. **Прокси включены**: 7 штук в PROXY_LIST, ротация работает; soundcloud-воркер поднят до **concurrency=3** (темп 3000/сутки, SPEC §4). **RU-прогон исследователя завершён**: +510 артистов (score<60 отсеян), attach-sources дал **570 SoundCloud + 163 YouTube** источников — уйдут в закачку по ежедневному таймеру. **Мировая очередь запущена** ([deploy/research_world.sh](deploy/research_world.sh), setsid, лог `/var/log/tg-music-research.log`): US 1200 → GB/DE/FR/SE/IT/ES/CA/AU/KR/JP/IN/BR/MX/UA/KZ/BY (~4000), attach-sources после каждой страны
- **Срез 6 задеплоен**: `tracks.artist_id` (миграция `227271058451`), новые импорты пишут artist_id сразу ([artist_binding.py](app/services/artist_binding.py) — нормализация в Python: SQLite lower() не берёт кириллицу), бэкфилл привязал **893 трека**; `python -m app.cli.artists bind-tracks` / `unbound`. Добивка биндинга после конца мировой очереди — вотчер на VPS (лог `/var/log/tg-music-bind.log`). ⚠️ pgrep-вотчеры по имени команды ловят сами себя — ждать только по PID (`kill -0`)
- **Срез 5а — автоплейлисты редакции**: [genre_playlists.py](app/services/genre_playlists.py) — подборка от первого админа на каждый жанр с ≥10 треками (идемпотентно, состав освежается), `python -m app.cli.genres make-playlists`; наполняет «Кураторов» Mini App. Первый прогон дал 0 — у артистов владельца не было жанров → добавлен `python -m app.cli.research enrich` (MusicBrainz по точному имени, score≥90) и **матч жанров/карточки переведён на artist_id** (SQLite lower() не понижает кириллицу — имя осталось фолбэком для непривязанных)
- **Пост-цепочка на VPS** (вотчер 453587 ждёт PID 452161 мировой очереди): enrich 1000 → bind-tracks → make-playlists, лог `/var/log/tg-music-post.log`. ⚠️ Два MusicBrainz-прогона параллельно нельзя — лимит 1 req/sec на IP, троттлинг у каждого процесса свой
- **Осталось из SPEC-КАТАЛОГ**: срез 5б (таблица `albums` + импорт SoundCloud sets как альбомов) — отложен, альбомы в карточке работают из `tracks.album`; периодический `bind-tracks`/`make-playlists` в ежедневный таймер; папка `проверка/` со скринами — сверка дизайна карточки, когда владелец скинет

## Checkpoint (2026-07-23) — МАССОВАЯ ЗАКАЧКА ЗАПУЩЕНА: 460 артистов, обложки, альбомы

- **Закачка идёт на проде**: 460 SoundCloud-профилей владельца ([артисты/ARTISTS.csv](артисты/ARTISTS.csv)) заведены артистами (таблица `artists`, миграция `8e5f9a2b3c4d`) и источниками (`soundcloud_sources`), скан запущен (`youtube scan-due`), первые треки Big Baby Tape уже в базе **с обложками**. Прокси владелец ещё не дал (`PROXY_LIST` пуст) — качаем с VPS-IP с impersonation и паузами; при бане — ждать прокси
...
- **Перенос музыки из Spotify / Яндекс.Музыки / ВК** ([playlist_transfer/](app/services/playlist_transfer/)): переносим МЕТАДАННЫЕ, не файлы (у сервисов DRM). Парсеры: Spotify — embed-страница (`__NEXT_DATA__`), Яндекс — `handlers/playlist.jsx` (JSON без авторизации), ВК и всё остальное — текстовый список «Артист — Название». Каждый трек: сначала ищем в своей базе (мгновенно в библиотеку), иначе `ytsearch1` + обычный user-импорт. Celery `transfer.playlist` (очередь youtube_user) + отчёт в чат. Точки входа: `/transfer`, кнопка в меню бота, экран Mini App (Настройки → Перенос из других сервисов), `POST /transfer`
- **Скорость Mini App**: boot = 2 лёгких запроса (главная сразу, каталог/библиотека фоном); рендер батчится микрозадачей и пропускается при неизменной разметке (⚠️ не rAF — не срабатывает в свёрнутом Mini App); `backdrop-filter` убран с карточек списков; `content-visibility/contain` на строках треков
- **Конверсия** (запрос «чтобы конвертировать всех»): пробный Premium 3 дня в один тап (`users.trial_used`, карточка на главной + баннер на экране Premium); 26 достижений, каждое даёт дни Premium (`user_achievements`, миграция `c0d77daffa52`, начисление при открытии профиля); рефералка — 10 порогов (неделя за первого друга), баннер «ещё N друзей», топ приглашающих `/referral/top`, тизер на главной
- **Плеер**: очередь с прыжком по тапу, таймер сна (⚙️), повтор трека, «Микс по треку», нижние табы Очередь/Текст
- **Эквалайзер** (Web Audio, 7 полос, 20 пресетов VK), акцентные цвета интерфейса, тактильный отклик, «Скачать всё», «Память устройства»
- **Профиль** по скринам: градиентная шапка с тегами исполнителей, «Топ артистов»/«Топ треков» из статистики (`/profile/top`)
- Тесты: **227 passed**

## Checkpoint (2026-07-21, стабильный стрим + VK-дизайн + рассылка)

Сессия по NEXT_SESSION.md: P1/P2/P3/P6/P7/P8 сделаны, P4/P5 ждут материалов владельца.
- **P1 «трек недоступен» устранён**: фронт не хранит `audio_url` в localStorage (протухал за 6ч), при ошибке/отсутствии ссылки перезапрашивает `GET /track/{id}` (отрицательный id = минус — единый эндпоинт). Минт через бота теперь кладёт архивную копию в хранилище (нет зависимости от лимита Bot API 20 МБ). LRU-кэш байтов на диске ([audio_cache.py](app/services/audio_cache.py), `AUDIO_CACHE_MAX_MB`=2048). CLI бэкфилла архива: `python -m app.cli.backfill_archives`. ⚠️ **На VPS полный бэкфилл НЕ запущен**: нужно 6.1 ГБ, свободно ~5.8 (диск 20 ГБ; вычищено 3.8 ГБ мусора yt-dlp из /tmp). Горячие треки держит LRU-кэш (`AUDIO_CACHE_MAX_MB=1024` в .env VPS), архивация при минте защищена от переполнения диска (`archive_min_free_gb`, дефолт 2 ГБ — ниже запаса копия просто не пишется). Полный архив — после S3/большего диска (SCALING.md)
- **P2 анти-бан SoundCloud 24/7**: `PROXY_LIST` (http/socks5 через запятую) — ротация по кругу на каждый запрос ([proxies.py](app/services/proxies.py)), при ошибке повтор через следующий; прокси только для SoundCloud-путей (YouTube на impersonation/прокси отвечал 403). Паузы `SOUNDCLOUD_MIN_DELAY=5`/`SOUNDCLOUD_MAX_DELAY=60`. ⛔ Сами адреса прокси даст владелец
