...
class Track(Base):
    ...
    # Ссылка источника, откуда трек приехал (страница SoundCloud/YouTube).
    # Нужна как точный ключ «этот кандидат из выдачи у нас уже есть»: сверка по
    # «исполнитель — название» промахивается, потому что в выдаче заголовок один,
    # а в скачанном файле — другой (источник дописывает «prod.», «Official Audio»
    # и прочее). Из-за промаха бот заново качал уже залитый трек: человек ждал
...
class MixHistory(Base):
    """Что Infinity Mix уже показывал пользователю — чтобы не крутить одно и то же по
