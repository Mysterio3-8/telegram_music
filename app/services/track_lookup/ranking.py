...
def rank_candidates(query: str, candidates: list[Candidate]) -> list[Candidate]:
    ...
    def order(row: tuple[float, Candidate]) -> tuple[int, float]:
        ...
        # ноль у него значит «неизвестно», а не «никто не слушал». Считая его
        # настоящим нулём, мы отнимали у каждого кандидата YouTube треть веса —
        # выше 0.6 он не мог подняться в принципе, и любой SoundCloud-реаплоад с
        # галочкой официальности обходил точное совпадение с YouTube.
        # Поэтому при неизвестной популярности её долю забирает похожесть.
