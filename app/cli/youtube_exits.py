...
# Все socks-входы Xray на сервере. 10811–10815 — первая партия узлов (из неё
# замер 16.08 оставил живым только 10813, он же 10808); 10821–10826 — вторая,
# добавленная владельцем в тот же день, из неё живы 10821–10824.
# ⚠️ Список здесь — только значение по умолчанию для ручного прогона. Что реально
# использует прод, задаёт YOUTUBE_PROXY в .env, и `--check` смотрит именно его.
_DEFAULT_PORTS = (10808, 10811, 10812, 10813, 10814, 10815,
...
def _configured_proxies() -> list[str]:
    """ВСЕ настроенные выходы, а не очередная попытка.

    ⚠️ Тут легко ошибиться, и я ошибся: `youtube_proxy_chain()` отдаёт не список
    выходов, а план ближайших попыток — `YOUTUBE_PROXY_ATTEMPTS` штук со сдвигом
    по кругу. Проверка на нём смотрела бы каждую ночь разные два выхода из пяти
    и рапортовала «жив», пока остальные три тихо мертвы.
    """
    from app.config import settings

    return list(settings.youtube_proxy_items)
...
def check() -> int:
    ...
    if not alive:
        ...
        return 1
    # Ждать, пока умрут все, — значит узнать об этом последним. Один живой из
    # нескольких настроенных это уже один шаг до полного отказа, и узлы обычно
    # выбывают не разом, а по очереди.
    if len(alive) == 1 and len(proxies) > 2:
        _alert(
