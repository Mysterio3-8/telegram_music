...
async def _maybe_send_cover(bot: Bot, chat_id: int, user: User, track: Track) -> None:
    """Обложка отдельной картинкой — теперь ВСЕГДА, если она у трека есть
    (решение владельца: экран настроек убран, обложка идёт по умолчанию).

    Документом, а не фото: Telegram пережимает фотографии, и обложка приезжала
    бы мыльной, тогда как смысл ровно в том, чтобы получить картинку целиком для
    своей медиатеки.
